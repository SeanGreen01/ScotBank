...
public class App extends Jooby {
    ...
    String fetchOAuth2Token() throws Exception {
        String clientSecret = System.getenv("SCOTBANK_API_CLIENT_SECRET");
        if (clientSecret == null || clientSecret.isBlank()) {
            throw new IllegalStateException("SCOTBANK_API_CLIENT_SECRET must be set for external API mode");
        }
        String authHeaderValue = "Basic " + Base64.getEncoder().encodeToString(("scotbank:" + clientSecret).getBytes(StandardCharsets.UTF_8));
    ...
    public void onStart() throws Exception {
        ...
        businessList.clear();
        if (isDemoMode()) {
            DemoData.populate(accounts, transactionsList, businessList);
            getLog().info("Using local demo data; no external API requests.");
        } else {
            String accessToken = fetchOAuth2Token();
            importAccDataFromAPI("https://api.asep-strath.co.uk/api/accounts", accessToken);
            importTransactionDataFromAPI("https://api.asep-strath.co.uk/api/transactions", accessToken, 100);
    ...
    public static boolean isDemoMode() {
        return Boolean.parseBoolean(System.getenv().getOrDefault("SCOTBANK_DEMO", "true"));
    }
